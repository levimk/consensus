...
func (node *RaftNode) AppendEntriesBroadcast() {
	...
	fmt.Println(fmt.Sprintf("\t[%d] heartbeat", node.Id))
...
func (peer *RaftPeer) AppendEntries(
	...
	clientIds := make([]string, len(entries))
	for _, entry := range entries {
		terms = append(terms, int32(entry.Term))
		indexes = append(indexes, int32(entry.Index))
		commands = append(commands, entry.Command.Command)
		values = append(commands, entry.Command.Value)
		clientIds = append(commands, entry.Command.ClientId)
	}
...
func (server *RaftNode) AppendEntries(
	...
	fmt.Println(fmt.Sprintf("\t[%d] received appendEntries", server.Id))
	...
	for i := 0; i < len(message.Terms); i++ {
		...
		clientId := message.ClientIds[i]
		newEntryCommand = types.Command{
			Command:  command,
			Value:    value,
			ClientId: clientId,
		}
		...
		if err != nil {
			...
			fmt.Println(fmt.Sprintf("[%d] added to log: client[%s] %s=%s", server.Id, clientId, command, value))
		}
	}
